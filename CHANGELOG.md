...
- Durable MCP requests created before ntfy publication are now removed only when ntfy authoritatively rejects that publish with a validation or rate-limit response; network and other delivery-ambiguous failures keep the pending record for safe recovery, and an already-terminal human response is never discarded.
- Human-visible choice labels now preserve valid UTF-16 at the existing 32-code-unit ntfy action-label boundary, so an astral Unicode character is never split into an unpaired surrogate while the full decision value and duplicate-label safeguards remain unchanged.

Pending, timeout, disconnect, setup/config failure, ambiguous transport failure, and notification delivery still never mean approval. Explicit Allow/Deny behavior, response-topic secrecy, first-terminal-wins semantics, notification authority, and the narrower remote Worker boundary are unchanged.
...
- Durable allowed decision values and already-resolved terminal decisions now normalize surrounding whitespace before persistence and membership validation, keeping semantically equivalent choices canonical.
- Exported direct-library terminal resolution now applies the same normalization to incoming response decisions before allowed-value matching, so padded-but-valid decisions resolve to their canonical value instead of failing early.
...
- Durable non-choice `refine` terminal decisions now require non-empty refinement text, preventing semantically incomplete resolved requests while preserving `refine` as an ordinary explicit choice value.
- Direct-library string choice options are now trimmed and validated like object-form choices: empty/whitespace-only and over-80-character values are rejected, and generated phone labels are bounded consistently to 32 characters.
...
- Agent-hook summary redaction now applies the existing secret-key classifier to URL fragment parameters as well as query parameters, preventing OAuth-style `#access_token=` and `#id_token=` values from escaping through outbound summaries while preserving benign fragment metadata.
...
- Agent-hook summary redaction now treats `authHeaderValue`, prefixed `*AuthHeaderValue`, and their plural `*AuthHeaderValues` object-field forms as credential-bearing secrets, preventing bearer/basic credentials from escaping through common header-value aliases while preserving non-secret metadata such as `authHeaderName`, `authMode`, and `authentication`.
...
- Agent-hook summary redaction now removes folded or multiline `Authorization` and `Proxy-Authorization` continuation values as one credential-bearing header, preventing Digest-, AWS-, Bearer-, Basic-, or similar continuation parameters from surviving after only the first line was redacted.
